...
- [open] ID:OA-101 Severity:medium Module:Oracle Aggregator V1.1 Owner:subagent-oa Summary:Wrapper heartbeat/threshold settings are governance-supplied and unbounded Notes:ORACLE_MANAGER_ROLE operators can widen `setHeartbeatStaleTimeLimit` or zero-out threshold guards on the API3/Redstone wrappers, effectively disabling liveness checks. Add deployment assertions for expected heartbeat windows and monitor on-chain events for unexpected changes.
...
- **Wrapper misconfiguration** – A compromised oracle manager can point an asset to a malicious wrapper address (`OracleAggregatorV1_1.setOracle`). Restrict the role to multi-sigs and monitor `OracleUpdated` events for unexpected routes.
- **Heartbeat disablement** – Wrappers expose `setHeartbeatStaleTimeLimit`; inflating the buffer effectively accepts stale data. Alert on large increases and codify expected limits per asset.
- **Threshold bypass** – Setting a threshold config with `lowerThresholdInBase = 0` neutralises the peg guard. Keep configuration playbooks and sanity checks that ensure thresholds remain enforced for wrapped stables.
- **Hard-peg misuse** – Mapping a volatile asset to `HardPegOracleWrapperV1_1` pins its price to the peg. Require explicit governance approval before routing new assets to the hard peg wrapper.
