...
contract UiPoolDataProviderV3 is IUiPoolDataProviderV3 {
  ...
  function getReservesData(
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      (reserveData.totalPrincipalStableDebt, , reserveData.averageStableRate, reserveData.stableDebtLastUpdateTimestamp) = IStableDebtToken(
      ...
      try DefaultReserveInterestRateStrategy(reserveData.interestRateStrategyAddress).getBaseStableBorrowRate() returns (uint256 res) {
        reserveData.baseStableBorrowRate = res;
      } catch {}
      try DefaultReserveInterestRateStrategy(reserveData.interestRateStrategyAddress).getBaseVariableBorrowRate() returns (uint256 res) {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      if (userConfig.isBorrowing(i)) {
        ...
        if (userReservesData[i].principalStableDebt != 0) {
          ...
          userReservesData[i].stableBorrowLastUpdateTimestamp = IStableDebtToken(baseData.stableDebtTokenAddress).getUserLastUpdated(user);
        }
      }
    }
