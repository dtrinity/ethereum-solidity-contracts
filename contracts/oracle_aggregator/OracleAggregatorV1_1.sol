 ...
 */
contract OracleAggregatorV1_1 is AccessControl, IOracleWrapperV1_1 {
    /* Core state */
    ...
    function setOracle(address asset, address oracle) external onlyRole(ORACLE_MANAGER_ROLE) {
        _assertCompatibleOracle(asset, oracle);
    ...
    function getPriceInfo(address asset) public view returns (uint256 price, bool isAlive) {
        ...
        return IOracleWrapperV1_1(oracle).getPriceInfo(asset);
    }

    function _assertCompatibleOracle(address asset, address oracle) private view {
        if (oracle == address(0)) {
            revert ZeroAddress("oracle");
        }
        if (oracle.code.length == 0) {
            revert OracleNotContract(oracle);
        }

        address oracleBaseCurrency = IOracleWrapperV1_1(oracle).BASE_CURRENCY();
        if (oracleBaseCurrency != baseCurrency) {
            revert UnexpectedBaseCurrency(asset, oracle, baseCurrency, oracleBaseCurrency);
        }
