...
/**
 * @title API3CompositeWrapperWithThresholdingV1_1
 * @dev Implementation of IAPI3Wrapper for composite API3 oracles with thresholding
 */
contract API3CompositeWrapperWithThresholdingV1_1 is BaseAPI3WrapperV1_1, ThresholdingUtils {
    /* Core state */
    ...
    function addCompositeFeed(
        ...
        if (proxy1 == address(0) || proxy2 == address(0)) {
            revert ProxyAddressZero();
        }
        if (proxy1.code.length == 0) {
            revert ProxyNotContract(proxy1);
        }
        if (proxy2.code.length == 0) {
            revert ProxyNotContract(proxy2);
        }
