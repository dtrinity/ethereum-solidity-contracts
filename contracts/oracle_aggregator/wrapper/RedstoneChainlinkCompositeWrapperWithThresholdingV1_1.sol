...
/**
 * @title RedstoneChainlinkCompositeWrapperWithThresholdingV1_1
 * @dev Implementation of BaseChainlinkWrapper for composite Redstone oracles with thresholding
 */
contract RedstoneChainlinkCompositeWrapperWithThresholdingV1_1 is BaseChainlinkWrapperV1_1, ThresholdingUtils {
    /* Core state */
    ...
    function addCompositeFeed(
        ...
        if (feed1.code.length == 0) {
            revert FeedNotContract(feed1);
        }
        if (feed2.code.length == 0) {
            revert FeedNotContract(feed2);
        }
