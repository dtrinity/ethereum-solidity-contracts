...
library DLoopCoreLogic {
  ...
  function isTooImbalanced(
    ...
    return
      currentLeverageBps != 0 && (currentLeverageBps < lowerBoundTargetLeverageBps || currentLeverageBps > upperBoundTargetLeverageBps);
  }
  ...
  function getCollateralTokenDepositAmountToReachTargetLeverage(
    ...
    requiredCollateralDepositAmountInBase = Math.ceilDiv(
      expectedTargetLeverageBps * (totalCollateralBase - totalDebtBase) - totalCollateralBase * BasisPointConstants.ONE_HUNDRED_PERCENT_BPS,
      denominator
    );
  ...
  function quoteRebalanceAmountToReachTargetLeverage(
    ...
    if (currentLeverageBps < targetLeverageBps) {
      ...
      inputTokenAmount = convertFromBaseCurrencyToToken(inputCollateralAmountInBase, collateralTokenDecimals, collateralTokenPriceInBase);
    ...
    // If the current leverage is above the target leverage, the user should decrease the leverage
      ...
      uint256 estimatedCollateralAmountInBase = getCollateralWithdrawAmountInBaseToDecreaseLeverage(inputDebtAmountInBase, subsidyBps);
  ...
  function getGrossAmountRequiredForNet(uint256 netAmount, uint256 withdrawalFeeBps) internal pure returns (uint256 grossAmount) {
    return
      Math.mulDiv(netAmount, BasisPointConstants.ONE_HUNDRED_PERCENT_BPS, BasisPointConstants.ONE_HUNDRED_PERCENT_BPS - withdrawalFeeBps);
  }
  ...
  function getNetAmountAfterFee(uint256 grossAmount, uint256 withdrawalFeeBps) internal pure returns (uint256 netAmount) {
    return
      Math.mulDiv(grossAmount, BasisPointConstants.ONE_HUNDRED_PERCENT_BPS - withdrawalFeeBps, BasisPointConstants.ONE_HUNDRED_PERCENT_BPS);
  }
}
