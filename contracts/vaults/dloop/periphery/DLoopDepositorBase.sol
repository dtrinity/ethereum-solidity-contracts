...
abstract contract DLoopDepositorBase is IERC3156FlashBorrower, Ownable, ReentrancyGuard, SwappableVault, RescuableVault {
  ...
  function calculateMinOutputShares(uint256 depositAmount, uint256 slippageBps, DLoopCoreBase dLoopCore) public view returns (uint256) {
    ...
    return
      Math.mulDiv(expectedShares, BasisPointConstants.ONE_HUNDRED_PERCENT_BPS - slippageBps, BasisPointConstants.ONE_HUNDRED_PERCENT_BPS);
  }
   ...
   */
  function _calculateEstimatedOverallSlippageBps(uint256 currentEstimatedShares, uint256 minOutputShares) internal pure returns (uint256) {
    /*
  ...
  function deposit(
    ...
    FlashLoanParams memory params = FlashLoanParams(receiver, assets, leveragedCollateralAmount, debtTokenToCollateralSwapData, dLoopCore);
    ...
    ERC20(debtToken).forceApprove(address(flashLender), maxFlashLoanAmount + flashLender.flashFee(address(debtToken), maxFlashLoanAmount));
  ...
  function _executeDepositAndValidate(
    ...
    if (debtTokenReceivedAfterDeposit < debtTokenAmountUsedInSwap + flashLoanFee) {
      revert DebtTokenReceivedNotMetUsedAmountWithFlashLoanFee(debtTokenReceivedAfterDeposit, debtTokenAmountUsedInSwap, flashLoanFee);
    }
  }
