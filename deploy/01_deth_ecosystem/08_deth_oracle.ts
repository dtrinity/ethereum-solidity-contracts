...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment): Promise<boolean> {
  ...
  await ensurePegConfigured(hardPegWrapper, dethAddress, pegValue);
  ...
  const assetConfig = await oracleAggregator.getAssetConfig(dethAddress);
  ...
  await ensureRiskConfig(oracleAggregator, dethAddress, assetConfig);

  try {
    await (await oracleAggregator.updateLastGoodPrice(dethAddress)).wait();
  } catch (error) {
    console.warn(`   ⚠️  Unable to prime last good price for dETH: ${(error as Error).message}`);
  }
