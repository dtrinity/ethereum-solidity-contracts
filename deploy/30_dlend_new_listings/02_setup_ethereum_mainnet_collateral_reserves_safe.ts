...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment): Promise<boolean> {
  ...
  const success = await executor.flush("Ethereum mainnet dLEND collateral reserve init + stage rollout (batch 1/2)");

  if (!success) {
    throw new Error("Failed to create Safe batch for collateral reserve init + stage rollout (batch 1/2).");
  }
...
func.id = "setup-ethereum-mainnet-collateral-reserves-safe-v8";
