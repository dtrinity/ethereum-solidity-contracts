...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment): Promise<boolean> {
  ...
  await executor.tryOrQueue(
    async () => {
      throw new Error("Direct execution disabled: queue Safe transaction.");
    },
  ...
  console.log("dlend-freeze-guardian-risk-admin-safe: complete");
  return true;
};

func.tags = ["post-deploy", "safe", "dlend", "freeze-guardian", "dlend-freeze-guardian-risk-admin-safe"];
func.dependencies = ["dlend-market", DLEND_FREEZE_GUARDIAN_ID];
