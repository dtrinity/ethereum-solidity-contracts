...
2. If dUSD was paused, Timelock unpauses it. Do **not** unfreeze dLEND. Unpause the legacy router, `setVaultConfigs` so Idle is the only Active 100% vault, `reinvestFees` leftover cash into Idle, pause the old router, re-pause dUSD, **then** `verifyLegacyCashHandled()`. Exact supply/backing conservation still required. Holder cash stays in sdUSD via Idle, not a Safe. Replacement cloned targets keep Idle in the graph (dLEND 100%); Idle delist is a later Timelock. A second successive Safe/Timelock later reopens (unpause/unfreeze) after verify.
...
- ACL `isPoolAdmin(timelock) = true` (also `isRiskAdmin = true`). Selected path: **Idle retarget + reinvest**, dLEND remains frozen. Never the freeze-only Safe. Unfreeze is a later reopen Timelock.
...
**dLEND wrap of leftover cash is not the selected path.** Fresh pin `25926563` failed `cash-backing-changed` after unfreeze+`reinvestFees` into stkdLEND (index remainder). Idle parking is selected so delay+1 and delay+2 both conserve backing. Do not widen the guard. Rehearse the actual intended state before CREATE.
