...
import {
  IncidentError, check, fail, canonical, digest, sameAddress, lower, ZERO, ZERO_HASH,
  PRIVILEGED_ROLES, validateInventory, migrationCalls, assertMigrationPlan, isLocalUrl,
} from "./policy.mjs";
...
const ABI = {
  ...
  asset: ["function balanceOf(address) view returns(uint256)", "function allowance(address,address) view returns(uint256)", "function paused() view returns(bool)", "function pause()"],
  token: ["function router() view returns(address)", "function collateralVault() view returns(address)", "function asset() view returns(address)", "function totalAssets() view returns(uint256)", "function totalSupply() view returns(uint256)", "function migrateCore(address,address)"],
  collateral: ["function dStakeToken() view returns(address)", "function dStable() view returns(address)", "function router() view returns(address)", "function getSupportedStrategyShares() view returns(address[])", "function setRouter(address)"],
  strategy: ["function asset() view returns(address)", "function balanceOf(address) view returns(uint256)", "function totalSupply() view returns(uint256)", "function totalAssets() view returns(uint256)", "function previewRedeem(uint256) view returns(uint256)"],
  adapter: ["function strategyShare() view returns(address)", "function collateralVault() view returns(address)", "function strategyShareValueInDStable(address,uint256) view returns(uint256)", "function setAuthorizedCaller(address,bool)"],
  router: [
    "function paused() view returns(bool)", "function pause()", "function dStakeToken() view returns(address)", "function collateralVault() view returns(address)",
    "function totalManagedAssets() view returns(uint256)", "function currentShortfall() view returns(uint256)",
    "function depositCap() view returns(uint256)", "function withdrawalFeeBps() view returns(uint256)", "function reinvestIncentiveBps() view returns(uint256)",
    "function dustTolerance() view returns(uint256)", "function maxVaultCount() view returns(uint256)", "function defaultDepositStrategyShare() view returns(address)",
    "function getVaultCount() view returns(uint256)", "function getVaultConfigByIndex(uint256) view returns(tuple(address strategyVault,address adapter,uint256 targetBps,uint8 status))",
    "function strategyShareToAdapter(address) view returns(address)", "function governanceModule() view returns(address)", "function rebalanceModule() view returns(address)",
    "function BACKING_GUARD_VERSION() view returns(uint256)",
  ],
  module: ["function moduleMetadata() view returns(bytes32,address,address)"],
  guard: ["function begin()", "function finish()", "function phase() view returns(uint8)", "function oldRouter() view returns(address)", "function newRouter() view returns(address)", "function token() view returns(address)", "function collateral() view returns(address)", "function timelock() view returns(address)", "function retiredDeployer() view returns(address)"],
  timelock: [
...
function parse(argv) {
  ...
  const allowed = new Set(["rpc-env", "config", "out", "deployer", "deployment", "plan", "review-sha256", "broadcast", "execute", "dry-run", "local-fork"]);
  ...
  while (argv.length) {
    ...
    else { check(argv.length > 0 && !argv[0].startsWith("--"), "Missing option value."); options[name] = argv.shift(); }
  }
...
function readJson(file) {
  try { return JSON.parse(fs.readFileSync(file, "utf8")); } catch { fail("A required JSON/artifact file could not be read or parsed."); }
}
...
const safe = (c, title, transactions, signer = c.governanceSafe) => ({
  version: "1.0", chainId: String(c.chainId), createdAt: Date.now(),
  meta: { name: title, description: "Unsigned incident proposal. Independently review and simulate. No reopening is included.", txBuilderVersion: "1.18.0", createdFromSafeAddress: signer },
...
async function connect(c, options) {
  ...
  if (options["local-fork"]) {
    ...
    check(metadata && metadata.forkedNetwork && Number(metadata.forkedNetwork.chainId) === Number(c.chainId), "Not a Hardhat fork of the configured target chain.");
...
async function inventory(c, provider) {
  ...
  const s = {
    chainId: c.chainId, blockNumber: block.number, blockHash: block.hash, timestamp: block.timestamp,
    tokenRouter: await t.router(tag), vaultRouter: await cv.router(tag), tokenCollateral: await t.collateralVault(tag), tokenAsset: await t.asset(tag),
    routerToken: await r.dStakeToken(tag), routerCollateral: await r.collateralVault(tag),
    vaultToken: await cv.dStakeToken(tag), vaultAsset: await cv.dStable(tag),
    paused: await r.paused(tag), assetPaused: await a.paused(tag),
    supply: String(await t.totalSupply(tag)), assets: String(await t.totalAssets(tag)), managed: String(await r.totalManagedAssets(tag)),
    shortfall: String(await r.currentShortfall(tag)), cap: String(await r.depositCap(tag)), fee: String(await r.withdrawalFeeBps(tag)),
    incentive: String(await r.reinvestIncentiveBps(tag)), dust: String(await r.dustTolerance(tag)), maxVaults: String(await r.maxVaultCount(tag)),
    defaultVault: await r.defaultDepositStrategyShare(tag), routerCash: String(await a.balanceOf(c.oldRouter, tag)),
    tokenAllowance: String(await a.allowance(c.token, c.oldRouter, tag)), delay: String(await tl.getMinDelay(tag)),
    supported: [...await cv.getSupportedStrategyShares(tag)], configs: [], code: {},
    authority: {
      tokenAdmin: await t.hasRole(ZERO_HASH, c.timelock, tag), vaultAdmin: await cv.hasRole(ZERO_HASH, c.timelock, tag),
  ...
  for (let i = 0; i < count; i++) {
    ...
    s.configs.push({
      vault: v.strategyVault, adapter: v.adapter, targetBps: String(v.targetBps), status: Number(v.status),
      asset: await strategy.asset(tag), adapterShare: await adapter.strategyShare(tag), adapterCollateral: await adapter.collateralVault(tag),
      mappedAdapter: await r.strategyShareToAdapter(v.strategyVault, tag), balance: String(balance),
      strategySupply: String(await strategy.totalSupply(tag)), strategyAssets: String(await strategy.totalAssets(tag)),
  ...
  const addresses = new Set([
    c.token, c.asset, c.collateral, c.oldRouter, c.timelock,
    await r.governanceModule(tag), await r.rebalanceModule(tag),
...
function artifact(key) {
  ...
  for (const [sourceName, input] of Object.entries(build.input.sources)) {
    ...
    check(fs.readFileSync(path.join(ROOT, sourceName), "utf8") === input.content, "Stale artifact: source differs from compiler input. Clean and recompile.");
  }
...
async function deploymentPlan(c, s, provider, deployer) {
  ...
  check(![c.token, c.collateral, c.timelock, c.oldRouter].some((x) => sameAddress(x, deployer)), "Deployer must be a separate account, not a protocol anchor.");
  ...
  for (const key of names) {
    const a = artifact(key); builds[key] = a.buildDigest;
    const args = key === "guard"
      ? [c.timelock, c.token, c.collateral, c.oldRouter, addresses.router, deployer]
...
async function deploy(c, s, provider, options, out) {
  ...
  console.log(`Prepared ${plan.transactions.length} transactions. No existing protocol authority or pointer is changed by this deployment.`);
  ...
  const journal = { ...plan.addresses, deployer: options.deployer, targetChainId: c.chainId, localFork: Boolean(options.execute), reviewSha256: review, builds: plan.builds, receipts: [], complete: false };
  ...
  try {
    for (let i = 0; i < plan.transactions.length; i++) {
      ...
      check(receipt && receipt.status === 1, "A deployment/bootstrap transaction failed. Stop; inspect the journal. Never wire a partial deployment.");
...
async function operation(c, provider, calls, label) {
  ...
  return {
    targets, values, datas, predecessor, salt, delay, id,
...
async function containment(c, s, provider, out) {
  check(s.authority.proposer && s.authority.executor && s.authority.routerPauser, "Current authority does not support the proposed router pause.");
  if (!s.paused) {
    ...
    write(path.join(out, "containment-execute.safe.json"), safe(c, "Execute legacy sdUSD router pause AFTER timelock maturity", [op.execute]));
  }
  if (!s.assetPaused) {
    ...
    write(path.join(out, "emergency-dUSD-pause.safe.json"), safe(c, "dUSD emergency pause — preserve/reuse existing Safe nonce and signatures", [call], c.emergencySafe));
  }
  console.log("Unsigned containment files prepared. Do not supersede a pending emergency-Safe transaction or assume signatures are interchangeable.");
}
...
async function verifyReplacement(c, d, provider, migrated) {
  ...
  check(sameAddress(await r.dStakeToken(), c.token) && sameAddress(await r.collateralVault(), c.collateral), "Replacement immutable anchors mismatch.");
  check(sameAddress(await r.governanceModule(), d.governance) && sameAddress(await r.rebalanceModule(), d.rebalance), "Replacement module pointers mismatch.");
  ...
  for (const [getter, expected] of Object.entries({ oldRouter: c.oldRouter, newRouter: d.router, token: c.token, collateral: c.collateral, timelock: c.timelock, retiredDeployer: d.deployer })) {
  ...
  if (migrated) {
    ...
    check(!await cv.hasRole(role("ROUTER_ROLE"), c.oldRouter) && await cv.hasRole(role("ROUTER_ROLE"), d.router), "Custody roles not migrated.");
...
async function migration(c, s, d, provider, out) {
  ...
  for (let i = 0; i < s.configs.length; i++) {
    ...
    check(sameAddress(v.strategyVault, old.vault) && sameAddress(v.adapter, old.adapter) && String(v.targetBps) === old.targetBps, "Replacement configuration differs from fresh legacy snapshot.");
  }
  for (const [getter, expected] of Object.entries({ withdrawalFeeBps: s.fee, depositCap: s.cap, reinvestIncentiveBps: s.incentive, dustTolerance: s.dust, maxVaultCount: s.maxVaults })) {
    check(String(await newRouter[getter]()) === expected, "Replacement economic settings differ from the fresh legacy snapshot.");
  }
  check(await contract(c.asset, "asset", provider).balanceOf(d.router) === 0n, "Replacement router has unexpected cash; reconcile before migration.");
  ...
  write(path.join(out, "migration-execute.safe.json"), safe(c, "Execute ATOMIC paused replacement ONLY AFTER legacy pause and timelock maturity", [op.execute]));
  console.log(`Migration review SHA-256: ${plan.reviewSha256}`);
  console.log("Legacy router MUST be paused before execution. All replacement strategies stay suspended; no unpause/unfreeze or LP restoration is included.");
}
...
async function simulate(plan, provider, options, out) {
  ...
  check(plan.operation.values.every((x) => BigInt(x) === 0n), "Unexpected ETH value in migration.");
  ...
  if (!options.execute) { console.log("Simulation dry-run: add --local-fork --execute to exercise and revert the local batch."); return; }
  ...
  try {
    ...
    if (!await old.paused()) {
      const stop = await operation(plan.config, provider, [{ to: plan.config.oldRouter, data: new E.Interface(ABI.router).encodeFunctionData("pause") }], `fork-containment-${snapshot}`);
      await send(stop.schedule);
      await provider.send("evm_increaseTime", [Number(stop.delay) + 1]); await provider.send("evm_mine", []);
    ...
    await provider.send("evm_increaseTime", [Number(op.delay) + 1]); await provider.send("evm_mine", []);
    ...
    write(path.join(out, "fork-simulation.json"), { reviewSha256, passed: true, executionHash: receipt.hash, beforeScheduling: before, afterExecution: after, note: "Assets may accrue during timelock delay. The on-chain guard checked EXACT equality across the migration transaction itself. Fork is reverted after this check; this is not a reopening approval." });
    console.log("Local atomic migration passed postconditions. Reverting the fork snapshot; this does not certify exploit non-profitability or reopening safety.");
  } finally {
...
async function main() {
  ...
  try {
    if (command === "simulate") {
      ...
      await simulate(plan, provider, options, out); return;
    }
    if (command === "verify") {
      ...
      write(path.join(out, "post-migration-verification.json"), { verifiedAt: new Date().toISOString(), deployment: d.router, pausedAndIsolated: true, reopeningApproved: false });
      return;
    }
    const s = await inventory(c, provider);
    write(path.join(out, "inventory.json"), s);
    if (command === "inventory") { console.log(`Recorded block-pinned inventory at block ${s.blockNumber}. This is not a source/runtime certification.`); return; }
    if (command === "containment") { await containment(c, s, provider, out); return; }
    if (command === "deploy") { await deploy(c, s, provider, options, out); return; }
    check(options.deployment, "--deployment is required.");
    const d = readJson(options.deployment);
    check(Boolean(d.localFork) === Boolean(options["local-fork"]), "A local rehearsal deployment cannot be presented as a live deployment.");
    await migration(c, s, d, provider, out);
  } finally { provider.destroy(); }
}
main().catch((error) => {
  // Never echo ethers request objects, RPC URLs, wallet errors or environment values.
  console.error(error instanceof IncidentError ? error.message : "Operation failed. Raw provider/wallet diagnostics were suppressed to avoid leaking credentials. Check local dependencies, compilation, RPC access and on-chain preconditions.");
  process.exitCode = 1;
});
